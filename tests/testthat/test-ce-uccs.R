...
test_that("'expenditure' is valid", {
  expect_error(
    ce_uccs(ce_hg(2015, "integrated"), "Funnies"),
    paste(
...
test_that("output is a character vector", {
  expect_type(ce_uccs(test_hg, "Pets"), "character")
})

test_that("all ucc's are 6 characters long", {
  expect_true(all(nchar(ce_uccs(test_hg, "Pets")) == 6))
})

test_that("all ucc's can be converted to numeric", {
  expect_true(all(!is.na(as.numeric(ce_uccs(test_hg, "Ham")))))
})
