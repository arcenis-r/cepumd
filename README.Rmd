...
```{r, prep_data, cache = TRUE, echo = TRUE, results = 'hide', message = FALSE, warning = FALSE}
pet_int <- ce_prepdata(
...
```{r, calc_means_by_urbn, cache = TRUE}
...
```{r calc_median_diary, cache = TRUE}
...
We can also use data from the Diary only to calculate medians by bls_urbn.
```{r calc_median_diary_by_group, cache = TRUE}
