...
functions, one will be created in the local temporary directory. Also, most of the functions used for downloading or preparing data now take a \code{ce_dir} argument to help with this. The major benefit of this change is that all files are now in one place and can be quickly cleaned up. An ancillary benefit is that it can make the workflow much simpler for quickly getting an expenditure estimate.
